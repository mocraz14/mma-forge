...
import { grappling } from '../../data';
import { TechniqueCard } from '../../components/TechniqueCard';
import { SearchBar } from '../../components/SearchBar';
import { CategoryFilter } from '../../components/CategoryFilter';
import { theme } from '../../lib/theme';

export default function GrapplingScreen() {
  const [q, setQ] = useState('');
  const [cat, setCat] = useState('All');
  const cats = ['All', ...Array.from(new Set(grappling.map(s => s.subcategory)))];
  const filtered = useMemo(() => grappling.filter(t =>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={{ padding: 16, paddingBottom: 0 }}>
        <Text style={styles.h1}>Grappling</Text>
        <Text style={styles.sub}>BJJ · Wrestling · Submissions</Text>
